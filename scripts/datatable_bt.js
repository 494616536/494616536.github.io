    	...
    	$("div.panel,ul.nav",container).each(function(index,item){
    		if($(item).find(table).size()==0){
    			...
    			if($(item).closest("div.dms-search div.tab-content",tableContainer).size()==0){
    				calHeight = calHeight - $(item).get(0).offsetHeight;
    				return;
    			}
    		}else{
...
//	            			table.bootstrapTable('refreshOptions',{height:calHeight,pageSize:calPageSizeByHeight(calHeight)});
	            			table.bootstrapTable('resetView',{height:calHeight,pageSize:calPageSizeByHeight(calHeight)});
        ...
        table.closest(dmsCommon.DMS_CLOSEST_DIV,tableContainer).find("div.query-btn i.fa-search").parent().each(function() {
        	if(!tableOldOptions.parentTable){
        		var that = this;
        		var unBindSearch = $(that).attr('data-unBindSearch');
        		if(unBindSearch && unBindSearch == "true"){
        			return;
        		}
        		//加载查询按钮的属性
...
//    		 table.bootstrapTable('resetView');
        ...
        if(optionsExtend.isFormParam){
        	optionsExtend.queryParams = function(params){
        		...
        		if(window.top.dmsData){
            		...
            		params["tokenAccount"] = tokenAccount;
        		}
            	return params;
            };
        }else{
        ...
        optionsExtend.onLoadError = function(status,resource){
        	...
        	if(optionsOld.onLoadError){
        		optionsOld.onLoadError(status, resource);
        	}
        	return;
        };
...
//			data : JSON.stringify({
//				type : 'table',
//				page : tableContainer.data("pageUrl"),
//				fields : i18nFields
//			})
//		});
    };
    ...
    function extendInputTextFormat(item,optionsExtend){
    	if(item.inputTextFormat){
			 item.formatter = function(value, row, index){
				 ...
				 if(item.inputTextFormat.multiLine){
					 templete = '<textarea id="'+item.inputField+index+'" name="'+item.inputField+index+'" data-inputName="'+item.inputField+'" type="text" class="form-control {[validateClass]}" {[validateAttr]} style="resize:none;width:'+(item.inputTextFormat.width||200)+'px">'+value+'</textarea>';
				 }
    ...
    function extendDefaultFormat(item,optionsExtend){
		...
		if(item.inputField==undefined){
		 ...
		 //多语言
		var pageLanguage = dmsCommon.getPageLanguage();
		if(pageLanguage == "en_US"){
			var text = item.title;
			if(text && text.length){
				item.title = dmsCommon.i18n(text);
			}
		}
   ...
   function returnAutoValueFormat(item,value, row, index){
    ...
    function returnHiddenFormat(item,value,row,index){
    	if(item.inputHiddenFormat){
    		...
    		var hiddenName = item.inputHiddenFormat.hiddenFieldName==undefined?item.inputField:item.inputHiddenFormat.hiddenFieldName;
    		
			return '<input id="'+hiddenName+index+'" name="'+hiddenName+'" type="hidden" value = "'+hiddenValue+'"/>';
    	}
    }
    /**
     * 删除一行的function
     ...
     * 获取选中行的index集合 
     */
    var getSelectionIndexs = function(){
    	var selects = table.bootstrapTable('getSelections');   
    	if(selects.length>0){
    		var indexs = new Array();
    		$.each(selects,function(i,item){
    			indexs.push(selects[i].index);
    		});
    		return indexs.join(",");
    	}else{
    		return "";
...
//		});
  ...
  //添加合计
    ...
    return {
        ...
        defaultInit:function(optionsExtend,options){
        	defaultInit(optionsExtend,options)
        },
        ...
        bootstrapTable:function(command,options){
        	return table.bootstrapTable(command,options);
        },
