var dmsDict = function() {
	...
	function initCheckBoxValue(valueArray,checkBoxObj,valueName,descName,checkBoxContainer){
		...
		$.each(valueArray,function(j,value){
			...
			//生成显示内容框
			checkBoxArray.push("<span id = '"+name+"val"+j+"'>" +(dictValue[descName]!=undefined&&isStringNull(dictValue[descName])==false ? dictValue[descName]: dictLabelName)+"</span></label>");
    	});
...
//		if($(checkBoxObj).attr("data-radioClickEvent") === "true"){
//			var events = jQuery._data($(checkBoxObj).get(0), "events")["click"];
//			$("input[type='checkbox']",$(checkBoxObj).parent()).click(events);
//		}
...
//		$(checkBoxObj).children().remove();
		...
		if($(checkBoxObj).attr("data-model")){
			...
			if(dictDataMap){
				var item = $(checkBoxObj).attr("data-item");
				var type = $(checkBoxObj).attr("data-type");
				type = (type==undefined?"dict":type);
				if(item){
					try{
						var json = (new Function("return " + item))();
						initCheckBoxValue(json,checkBoxObj,"value","label",checkBoxContainer);
					}catch(e){
	...
	function initRadioValue(valueArray,radioBoxObj,valueName,descName){
		...
		var readonly = $(radioBoxObj).attr("readonly");
		if(readonly!=undefined){
			var _tmpRadio = $($(radioBoxObj).prop("outerHTML").replace("readonly", "onclick='return false' readonly"));
			$(radioBoxObj).replaceWith(_tmpRadio);
			radioBoxObj = _tmpRadio.get(0);
		}
		...
		$.each(valueArray,function(j,value){
			...
			var htmlStr = "<input type=radio name='"+name+"' id='"+(id+j)+"' "+(readonly!=undefined?"onclick='return false'":"")+" "+(inputName!=undefined?"data-inputName='"+inputName+"'":"")+" value='"+dataValue[valueName]+"' >";
			if(j==0){
		...
		if($(radioBoxObj).attr("data-radioClickEvent") === "true"){
//			var events = jQuery._data($(radioBoxObj).get(0), "events");
//			console.dir(events);
//			$("input[type='radio']",$(radioBoxObj).parent()).click(events);
		}
	...
	function initRedio(radioBoxObj){
		...
		if(model){
			...
			//初始化Redio Value
			if(dictDataMap){
				var item = $(radioBoxObj).attr("data-item");
				var type = $(radioBoxObj).attr("data-type");
				type = (type==undefined?"dict":type);
				if(item){
					try{
						var json = (new Function("return " + item))();
						initRadioValue(json,radioBoxObj,"value","label");
					}catch(e){
						console.log(e.message);
					}
				}else{
	...
	function initSelectValue(valueArray,selectObj,valueName,descName){
		...
		var excludeItems = $(selectObj).attr("data-excludeItems");
		var includeItems = $(selectObj).attr("data-includeItems");
		if(includeItems){
			var _excludeItems = [];
			$.each(valueArray, function(i,op){
				if(includeItems.indexOf(op[valueName]) == -1){
					_excludeItems.push(op[valueName]);
				}
			});
			excludeItems = _excludeItems.toString()
		}
		//展示下拉框值
		if(valueArray){
			if($.isArray(valueArray)){
				$.each(valueArray, function(j, value) {
					//初始化某一个option值
					initSelectOneOption(selectObj,valueArray[j],valueName,descName,excludeItems);
				});
			}else  if (typeof (valueArray)=="object"){
				for(var key in valueArray){
					//初始化某一个option值
					initSelectOneOption(selectObj,valueArray[key],valueName,descName,excludeItems);
			    }  
			}
		}
	...
	function modeUrl(selectObj){
		...
		if(cacheData != undefined){
			...
			//查询条件强制同步
			if($(selectObj).closest("div.dms-search").find("form:first").size()>0){
				dataAjaxSync = "true";
			}
			dmsCommon.ajaxRestRequest({
				url : dmsCommon.getDmsPath()[model] + url,
	...
	function initSelectData(selectObj){
		if($(selectObj).attr("data-model")){
			...
			if(dictDataMap){
				...
				if($(selectObj).find("option").length && $(selectObj).find("option:first").val()){
					dictValueArray = [];
					$(selectObj).find("option").each(function(){
						dictValueArray.push({"option_text":$(this).text(),"option_value":$(this).val()});
					});
					initSelectValue(dictValueArray,selectObj,"option_value","option_text");
					type = "option";
				}
				if(type=="dict"){
...
//			return ($("label.checkbox-inline",$(obj).parent().parent()).size()>0);
	 ...
	 * 根据ID 获取对应的选项
	 */
	var getCodeItemById = function(type,codeType){
	...
	return {
		...
		getCodeItemById:function(type,codeType){
			return getCodeItemById(type,codeType);
		},
